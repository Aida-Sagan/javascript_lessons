...
async function serachRecopeByIngredient() {
    const ingredient = ingredientInput.value.trim();
    try {
        const response = await fetch(`${API_URL}filter.php?i=${ingredient}`);
        const data = await response.json();
        console.log('data from serachRecopeByIngredient', data);
...
searchRecipeBtn.addEventListener('click', serachRecopeByIngredient)